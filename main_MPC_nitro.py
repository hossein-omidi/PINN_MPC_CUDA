...
from MPC_nitro import cost_fun_mimo
...
def run_mpc_simulation():
    ...
    dt_mpc = 0.02  # 5Hz control updates
    total_time = 100  # 10 minutes simulation
    ...
    # Proposed Changes
    W = np.diag([500, 500, 1])  # Reduced tracking weights for better numerical stability
    R = np.diag([1, 2, 4])  # Increased control penalties
        ...
        if t >= next_mpc_step:
                try:
                    ...
                    current_control, pred, _ = cost_fun_mimo(
                        ...
                        w_control_con=1e6,
                        s=1e-3,
                        horizon=50,  # Explicitly add horizon parameter
                        dt=dt_mpc,
                        max_iter=500  # Now correctly assigned
                    )
